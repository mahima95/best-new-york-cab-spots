...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = {
    ...
    //Create df based on query order by rectangle
    //Stored results in HotzoneResults view
    val hotzoneResultDf = spark.sql("select rectangle,count(point) as Count from joinResult group by rectangle order by rectangle").persist()
    hotzoneResultDf.createOrReplaceTempView("HotzoneResults")
    return hotzoneResultDf
  }
